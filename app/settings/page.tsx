    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <section className="mb-10">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl divide-y divide-zinc-800">
          ...
          <div className="flex items-center justify-between px-4 py-3">
            <div>
              <p className="text-sm font-medium text-zinc-200">Daily goal</p>
              <p className="text-xs text-zinc-500">Target tracked hours per day (shown in Reports)</p>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="number"
                ...
                max={24}
                step={0.5}
                value={settings.dailyGoalHours ?? 0}
                onChange={(e) => {
                  const val = Math.max(0, Math.min(24, Number(e.target.value)));
                  setSettings((s) => ({ ...s, dailyGoalHours: val }));
                }}
                className="w-20 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-zinc-100 text-sm text-right focus:outline-none focus:ring-2 focus:ring-orange-500/50"
              />
              <span className="text-sm text-zinc-500">h</span>
