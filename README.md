...
- ✅ Keyboard shortcuts — T/L/R to navigate, **C to resume most recent entry (Timer page)**, **F to enter/exit focus mode (Timer page, when running)**, **S to open stand-up summary (Log page)**, Space to toggle timer, N to log time, J to open today's day note, G to toggle project grouping, V to toggle timeline, / to focus search, ? for help modal, ⌘K for command palette
...
- ✅ Stand-up summary generator - "Stand-up" button on the Log page (or press `S`) opens a modal that generates a formatted "what I worked on" summary from yesterday, today, or both; three output formats: Markdown (GitHub/Linear/Notion), Plain bullets (email/notes), and Slack/Discord; visual preview tab + raw text tab; one-click copy to clipboard; auto-strips session annotation noise from descriptions; works off existing entries with zero configuration
